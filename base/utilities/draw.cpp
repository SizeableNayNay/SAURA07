...
void D::Setup(IDirect3DDevice9* pDevice, unsigned int uFontFlags)
{
	...
	style.WindowTitleAlign = ImVec2(0.033f, 0.5f);
	...
	F::Whitney = io.Fonts->AddFontFromMemoryCompressedTTF(whitney_compressed_data, whitney_compressed_size, 14.f, &imWhitneyConfig, io.Fonts->GetGlyphRangesCyrillic());

	ImFontConfig imVerdanaSmallConfig;
	imVerdanaSmallConfig.RasterizerFlags = ImGuiFreeType::Bold;
	F::WhitneySmall = io.Fonts->AddFontFromFileTTF(XorStr("C:\\Windows\\Fonts\\Verdana.ttf"), 12.f, &imVerdanaSmallConfig, io.Fonts->GetGlyphRangesCyrillic());
	...
	ImFontConfig imVerdanaESPConfig;
	imVerdanaESPConfig.RasterizerFlags = ImGuiFreeType::ForceAutoHint;
	F::VerdanaESP = io.Fonts->AddFontFromFileTTF(XorStr("C:\\Windows\\Fonts\\Verdana.ttf"), 14.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
	...
	F::Icons = io.Fonts->AddFontFromMemoryCompressedTTF(icons_compressed_data, icons_compressed_size, 40.f, &imIconsConfig, wIconRanges);
...
void D::SetupColors()
{
	...
	/* define menu colors */
	// background colors
	ImVec4 vecBackground = ImVec4(0.04f, 0.04f, 0.04f, 1.f);
	ImVec4 vecSecondaryBackground = vecBackground + ImVec4(0.08f, 0.08f, 0.08f, 0.f);

	// foreground colors
	ImVec4 vecForeground = ImVec4(0.95f, 0.95f, 0.95f, 1.f);

	// highlight colors
	ImVec4 vecHighlights = C::Get<ImVec4>(Vars.vecColorHighlights);
	ImVec4 vecSecondaryHighlights = vecHighlights;

	// halves the color saturation of the main color and stores it in seconardy variables
	vecSecondaryHighlights.x = vecSecondaryHighlights.x / 2.f;
	vecSecondaryHighlights.y = vecSecondaryHighlights.y / 2.f;
	vecSecondaryHighlights.z = vecSecondaryHighlights.z / 2.f;

	/* set menu colors */
	// text colors
	style.Colors[ImGuiCol_Text] = vecForeground;															// Foreground
	style.Colors[ImGuiCol_TextDisabled] = vecForeground - ImVec4(0.f, 0.f, 0.f, 0.2f);						// Foreground Slightly Transparent
	style.Colors[ImGuiCol_TextSelectedBg] = vecSecondaryHighlights;											// Secondary Highlight

	// window colors
	style.Colors[ImGuiCol_WindowBg] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);							// Background Slightly Transparent
	style.Colors[ImGuiCol_ChildBg] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.4f);							// Background Partially Transparent
	style.Colors[ImGuiCol_PopupBg] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.4f);							// Background Partially Transparent

	// border colors
	style.Colors[ImGuiCol_Border] = vecBackground;															// Background
	style.Colors[ImGuiCol_BorderShadow] = vecBackground;													// Background

	// frame colors
	style.Colors[ImGuiCol_FrameBg] = vecBackground;															// Background
	style.Colors[ImGuiCol_FrameBgHovered] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);					// Background Slightly Transparent
	style.Colors[ImGuiCol_FrameBgActive] = vecHighlights;													// Highlight

	// control colors
	style.Colors[ImGuiCol_ControlBg] = vecBackground;														// Background
	style.Colors[ImGuiCol_ControlBgHovered] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);					// Background Slightly Transparent
	style.Colors[ImGuiCol_ControlBgActive] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.8f);					// Background Transparent

	// title colors
	style.Colors[ImGuiCol_TitleBg] = vecForeground;															// Foreground
	style.Colors[ImGuiCol_TitleBgActive] = vecForeground;													// Foreground
	style.Colors[ImGuiCol_TitleBgCollapsed] = vecForeground;												// Foreground

	// menubar color
	style.Colors[ImGuiCol_MenuBarBg] = vecBackground;														// Background

	// scrollbar colors
	style.Colors[ImGuiCol_ScrollbarBg] = vecSecondaryBackground;											// Secondary Background
	style.Colors[ImGuiCol_ScrollbarGrab] = vecHighlights;													// Higlight
	style.Colors[ImGuiCol_ScrollbarGrabHovered] = vecSecondaryBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);		// Secondary Background Slightly Transparent
	style.Colors[ImGuiCol_ScrollbarGrabActive] = vecHighlights;												// Highlight

	// checkmark color
	style.Colors[ImGuiCol_CheckMark] = vecHighlights;														// Highlight

	// slider colors
	style.Colors[ImGuiCol_SliderGrab] = vecHighlights;														// Highlight
	style.Colors[ImGuiCol_SliderGrabActive] = vecSecondaryBackground;										// Secondary Background

	// button colors
	style.Colors[ImGuiCol_Button] = vecBackground;															// Background
	style.Colors[ImGuiCol_ButtonHovered] = vecSecondaryBackground;											// Secondary Background
	style.Colors[ImGuiCol_ButtonActive] = vecHighlights;													// Highlight

	// header colors
	style.Colors[ImGuiCol_Header] = vecHighlights;															// Highlight
	style.Colors[ImGuiCol_HeaderHovered] = vecSecondaryBackground;											// Secondary Background
	style.Colors[ImGuiCol_HeaderActive] = vecBackground;													// Background Slightly Transparent

	// separator colors
	style.Colors[ImGuiCol_Separator] = vecHighlights;														// Highlight
	style.Colors[ImGuiCol_SeparatorHovered] = vecHighlights;												// Highlight
	style.Colors[ImGuiCol_SeparatorActive] = vecHighlights;													// Highlight

	// resize grip colors
	style.Colors[ImGuiCol_ResizeGrip] = vecHighlights;														// Highlight
	style.Colors[ImGuiCol_ResizeGripHovered] = vecSecondaryBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);		// Secondary Background Slightly Transparent
	style.Colors[ImGuiCol_ResizeGripActive] = vecBackground;												// Background

	// tab colors
	style.Colors[ImGuiCol_Tab] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);								// Background Slightly Transparent
	style.Colors[ImGuiCol_TabHovered] = vecBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);						// Background Slightly Transparent
	style.Colors[ImGuiCol_TabActive] = vecHighlights;														// Highlight
	style.Colors[ImGuiCol_TabUnfocused] = vecSecondaryBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);				// Secondary Background Slightly Transparent
	style.Colors[ImGuiCol_TabUnfocusedActive] = vecHighlights - ImVec4(0.f, 0.f, 0.f, 0.2f);				// Highlight Slightly Transparent

	// plotline colors
	style.Colors[ImGuiCol_PlotLines] = vecHighlights;														// Highlight
	style.Colors[ImGuiCol_PlotLinesHovered] = vecHighlights - ImVec4(0.f, 0.f, 0.f, 0.4f);					// Highlight Partially Transparent
	style.Colors[ImGuiCol_PlotHistogram] = vecHighlights;													// Highlight
	style.Colors[ImGuiCol_PlotHistogramHovered] = vecHighlights - ImVec4(0.f, 0.f, 0.f, 0.4f);				// Highlight Partially Transparent

	// misc colors
	style.Colors[ImGuiCol_DragDropTarget] = vecSecondaryBackground - ImVec4(0.f, 0.f, 0.f, 0.2f);			// Secondary Background Slightly Transparent
	style.Colors[ImGuiCol_Triangle] = vecHighlights;														// Highlight

	style.Colors[ImGuiCol_ModalWindowDimBg] = vecBackground;												// Background
	#pragma endregion
}
