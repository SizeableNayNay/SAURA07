...
void CTriggerBot::Run(CUserCmd* pCmd, CBaseEntity* pLocal)
{
	...
	if (C::Get<int>(Vars.iTriggerbotKey) > 0 && !IPT::IsKeyDown(C::Get<int>(Vars.iTriggerbotKey)))
	{
		timer.Reset();
		return;
	}
	...
	Trace_t trace = { };
	if (C::Get<bool>(Vars.bTriggerbotAutoWall))
	{
		...
		if (flDamage < C::Get<int>(Vars.iTriggerbotMinimalDamage))
			return;
	...
	// hitgroup filters check
		// head
	if ((C::Get<bool>(Vars.bTriggerbotHead) && trace.iHitGroup == HITGROUP_HEAD) ||
		// chest
		(C::Get<bool>(Vars.bTriggerbotChest) && trace.iHitGroup == HITGROUP_CHEST) ||
		// stomach
		(C::Get<bool>(Vars.bTriggerbotStomach) && trace.iHitGroup == HITGROUP_STOMACH) ||
		// arms
		(C::Get<bool>(Vars.bTriggerbotArms) && (trace.iHitGroup == HITGROUP_LEFTARM || trace.iHitGroup == HITGROUP_RIGHTARM)) ||
		// legs
		(C::Get<bool>(Vars.bTriggerbotLegs) && (trace.iHitGroup == HITGROUP_LEFTLEG || trace.iHitGroup == HITGROUP_RIGHTLEG)))
	{
