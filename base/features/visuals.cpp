...
#pragma region visuals_store_esp
	...
	for (const auto& [pEntity, flDistance] : vecOrder)
	{
		...
		case EClassIndex::CCSPlayer:
		{
			...
			if ((!C::Get<bool>(Vars.bEspMainAllies) && !pEntity->IsEnemy(pLocal)) || (!C::Get<bool>(Vars.bEspMainEnemies) && pEntity->IsEnemy(pLocal)))
				break;
			...
			if (pEntity != pLocal)
			{
				...
				if ((C::Get<bool>(Vars.bEspMainAllyFarRadar) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyFarRadar) && pEntity->IsEnemy(pLocal)))
...
bool CVisuals::Chams(CBaseEntity* pLocal, DrawModelResults_t* pResults, const DrawModelInfo_t& info, matrix3x4_t* pBoneToWorld, float* flFlexWeights, float* flFlexDelayedWeights, const Vector& vecModelOrigin, int nFlags)
{
	...
	if (pEntity->IsPlayer() && pEntity->IsAlive() && (C::Get<bool>(Vars.bEspChamsEnemies) || C::Get<bool>(Vars.bEspChamsAllies)))
	{
		...
		{
			if (pEntity == pLocal && C::Get<bool>(Vars.bEspChamsSelf))
			{
				static IMaterial* pMaterialSelf = nullptr;
				static IMaterial* pMaterialDesync = nullptr;
				static IMaterial* pMaterialFakelag = nullptr;

				// set players material
				switch (C::Get<int>(Vars.iEspChamsPlayerSelf))
				{
				case (int)EVisualsPlayersChams::FLAT:
					pMaterialSelf = arrMaterials.at(0).second;
					break;
				case (int)EVisualsPlayersChams::REFLECTIVE:
					pMaterialSelf = arrMaterials.at(2).first;
					break;
				default:
					pMaterialSelf = arrMaterials.at(0).first;
					break;
				}

				// set players material
				switch (C::Get<int>(Vars.iEspChamsPlayerDesync))
				{
				case (int)EVisualsPlayersChams::FLAT:
					pMaterialDesync = arrMaterials.at(0).second;
					break;
				case (int)EVisualsPlayersChams::REFLECTIVE:
					pMaterialDesync = arrMaterials.at(2).first;
					break;
				default:
					pMaterialDesync = arrMaterials.at(0).first;
					break;
				}

				// set players material
				switch (C::Get<int>(Vars.iEspChamsPlayerFakelag))
				{
				case (int)EVisualsPlayersChams::FLAT:
					pMaterialFakelag = arrMaterials.at(0).second;
					break;
				case (int)EVisualsPlayersChams::REFLECTIVE:
					pMaterialFakelag = arrMaterials.at(2).first;
					break;
				default:
					pMaterialFakelag = arrMaterials.at(0).first;
					break;
				}

				// check is valid material
				if (pMaterialSelf == nullptr || pMaterialSelf->IsErrorMaterial() ||
					pMaterialDesync == nullptr || pMaterialDesync->IsErrorMaterial() ||
					pMaterialFakelag == nullptr || pMaterialFakelag->IsErrorMaterial())
					return false;

				// get colors
				const Color colSelf = C::Get<Color>(Vars.colEspChamsPlayer);
				const Color colFake = C::Get<Color>(Vars.colEspChamsPlayerDesync);
				const Color colFakeLag = C::Get<Color>(Vars.colEspChamsPlayerFakelag);

				// do chams for desync
				if (C::Get<bool>(Vars.bEspChamsPlayerDesync) && G::bIsBreaking && C::Get<bool>(Vars.bAntiAim))
				{
					// set fake color
					I::StudioRender->SetColorModulation(colFake.Base().data());

					// set alpha
					I::StudioRender->SetAlphaModulation((pEntity == pLocal && pLocal->IsScoped() && I::Input->bCameraInThirdPerson) ? 0.3f : colFake.aBase());

					// disable "$ignorez" flag
					pMaterialDesync->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);

					// set xqz wireframe
					pMaterialDesync->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsPlayerDesync) == (int)EVisualsPlayersChams::WIREFRAME ? true : false);

					// override ignorez material
					I::StudioRender->ForcedMaterialOverride(pMaterialDesync);

					// draw model with fake material
					oDrawModel(I::StudioRender, 0, pResults, info, CAntiAim::Get().pFakeBones, flFlexWeights, flFlexDelayedWeights, vecModelOrigin, nFlags);
				}

				// do chams for fakelag
				if (C::Get<bool>(Vars.bEspChamsPlayerFakelag) && (C::Get<bool>(Vars.bAntiAim) && G::bIsBreaking || C::Get<bool>(Vars.bMiscFakelag)))
				{
					// set fakelag color
					I::StudioRender->SetColorModulation(colFakeLag.Base().data());

					// set alpha
					I::StudioRender->SetAlphaModulation((pEntity == pLocal && pLocal->IsScoped() && I::Input->bCameraInThirdPerson) ? 0.3f : colFakeLag.aBase());

					// disable "$ignorez" flag
					pMaterialFakelag->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);

					// set xqz wireframe
					pMaterialFakelag->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsPlayerFakelag) == (int)EVisualsPlayersChams::WIREFRAME ? true : false);

					// override ignorez material
					I::StudioRender->ForcedMaterialOverride(pMaterialFakelag);

					// draw model with fakelag material
					oDrawModel(I::StudioRender, 0, pResults, info, CAntiAim::Get().pFakelagBones, flFlexWeights, flFlexDelayedWeights, vecModelOrigin, nFlags);
				}

				if (C::Get<bool>(Vars.bEspChamsPlayerSelf))
				{
					// do visible chams
					// set color
					I::StudioRender->SetColorModulation(colSelf.Base().data());

					// set alpha
					I::StudioRender->SetAlphaModulation((pEntity == pLocal && pLocal->IsScoped() && I::Input->bCameraInThirdPerson) ? 0.3f : colSelf.aBase());

					// disable "$ignorez" flag
					pMaterialSelf->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);

					// set wireframe
					pMaterialSelf->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsPlayerSelf) == (int)EVisualsPlayersChams::WIREFRAME ? true : false);

					// override customized material
					I::StudioRender->ForcedMaterialOverride(pMaterialSelf);
				}
			...
			{
				static IMaterial* pMaterialEnemies = nullptr;
				static IMaterial* pMaterialAllies = nullptr;

				// set players material
				switch (C::Get<int>(Vars.iEspChamsEnemies))
				{
				case (int)EVisualsPlayersChams::FLAT:
					pMaterialEnemies = arrMaterials.at(0).second;
					break;
				case (int)EVisualsPlayersChams::REFLECTIVE:
					pMaterialEnemies = arrMaterials.at(2).first;
					break;
				default:
					pMaterialEnemies = arrMaterials.at(0).first;
					break;
				}

				// set players material
				switch (C::Get<int>(Vars.iEspChamsAllies))
				{
				case (int)EVisualsPlayersChams::FLAT:
					pMaterialAllies = arrMaterials.at(0).second;
					break;
				case (int)EVisualsPlayersChams::REFLECTIVE:
					pMaterialAllies = arrMaterials.at(2).first;
					break;
				default:
					pMaterialAllies = arrMaterials.at(0).first;
					break;
				}

				// check is valid material
				if (pMaterialEnemies == nullptr || pMaterialEnemies->IsErrorMaterial() ||
					pMaterialAllies == nullptr || pMaterialAllies->IsErrorMaterial())
					return false;
				...
				if (pLocal->IsEnemy(pEntity) && C::Get<bool>(Vars.bEspChamsXQZEnemies) || !pLocal->IsEnemy(pEntity) && C::Get<bool>(Vars.bEspChamsXQZAllies))
				{
					// set xqz color
					...
					pLocal->IsEnemy(pEntity) ? pMaterialEnemies->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, true) : pMaterialAllies->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, true);

					// set xqz wireframe
					pLocal->IsEnemy(pEntity) ? pMaterialEnemies->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsEnemies) == (int)EVisualsPlayersChams::WIREFRAME ? true : false) :
						pMaterialAllies->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsAllies) == (int)EVisualsPlayersChams::WIREFRAME ? true : false);

					// override ignorez material
					pLocal->IsEnemy(pEntity) ? I::StudioRender->ForcedMaterialOverride(pMaterialEnemies) : I::StudioRender->ForcedMaterialOverride(pMaterialAllies);
	...
	else if (szModelName.find(XorStr("sleeve")) != std::string_view::npos && C::Get<bool>(Vars.bEspChamsPlayerViewModel) && C::Get<bool>(Vars.bEspChamsSelf) && C::Get<int>(Vars.iEspChamsPlayerViewModel) == (int)EVisualsPlayerViewModelChams::NO_DRAW)
	{
	...
	else if ((szModelName.find(XorStr("weapons\\v_")) != std::string_view::npos || szModelName.find(XorStr("arms")) != std::string_view::npos) && C::Get<bool>(Vars.bEspChamsPlayerViewModel) && C::Get<bool>(Vars.bEspChamsSelf))
	{
		...
		if (C::Get<int>(Vars.iEspChamsPlayerViewModel) == (int)EVisualsPlayerViewModelChams::NO_DRAW)
		{
		...
		switch (C::Get<int>(Vars.iEspChamsPlayerViewModel))
		{
		case (int)EVisualsPlayerViewModelChams::FLAT:
			pMaterial = arrMaterials.at(1).second;
			break;
		case (int)EVisualsPlayerViewModelChams::GLOW:
			pMaterial = arrMaterials.at(2).second;
			break;
		case (int)EVisualsPlayerViewModelChams::SCROLL:
			pMaterial = arrMaterials.at(3).first;
			break;
		case (int)EVisualsPlayerViewModelChams::CHROME:
		...
		// get color
		const Color colAdditional = C::Get<Color>(Vars.colEspChamsPlayerViewModelAdditional);
		const Color colViewModel = C::Get<Color>(Vars.colEspChamsPlayerViewModel);

		// change material variables
		if (C::Get<int>(Vars.iEspChamsPlayerViewModel) == (int)EVisualsPlayerViewModelChams::GLOW)
		{
			...
			// set wireframe
			pMaterial->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, C::Get<int>(Vars.iEspChamsPlayerViewModel) == (int)EVisualsPlayerViewModelChams::WIREFRAME ? true : false);
...
void CVisuals::Glow(CBaseEntity* pLocal)
{
	for (int i = 0; i < I::GlowManager->vecGlowObjectDefinitions.Count(); i++)
	{
		...
		// set bloom state
		hGlowObject.bFullBloomRender = ((C::Get<bool>(Vars.bEspGlowBloomEnemies) && pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspGlowBloomAllies) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspGlowBloomSelf) && pEntity->IsPlayer()));
		...
		case EClassIndex::CCSPlayer:
		{
			...
			if ((pLocal->IsEnemy(pEntity) && C::Get<bool>(Vars.bEspGlowEnemies)) ||
				// teammates & local
				(((pEntity == pLocal && I::Input->bCameraInThirdPerson && C::Get<bool>(Vars.bEspGlowSelf)) || !pLocal->IsEnemy(pEntity)) && C::Get<bool>(Vars.bEspGlowAllies)))
			{
				hGlowObject.Set(
					pEntity == pLocal ?
					pLocal->IsVisible(pEntity, pEntity->GetBonePosition(BONE_HEAD).value_or(pEntity->GetEyePosition(false))) ? C::Get<Color>(Vars.colEspGlowSelf) : C::Get<Color>(Vars.colEspGlowSelfWall) :
...
void CVisuals::HitMarker(const ImVec2& vecScreenSize, float flServerTime, Color colLines, Color colDamage)
{
	...
	for (std::size_t i = 0U; i < vecHitMarks.size(); i++)
	{
		...
		if (ImVec2 vecScreen = { }; D::WorldToScreen(vecHitMarks.at(i).vecPosition, vecScreen))
		{
			...
			// draw dealt damage
			D::AddText(F::VerdanaESP, 24.f, ImVec2(vecScreen.x, vecScreen.y - flRatio * flDistance), std::to_string(vecHitMarks.at(i).iDamage), colDamage, DRAW_TEXT_OUTLINE, Color(0, 0, 0, iAlpha));
		}
	}
}
...
void CVisuals::DroppedWeapons(CBaseCombatWeapon* pWeapon, short nItemDefinitionIndex, Context_t& ctx, const float flDistance, const Color& colPrimary, const Color& colAmmo, const Color& colBackground, const Color& colOutline)
{
	...
	if (C::Get<bool>(Vars.bEspMainWeaponDistance))
	{
		...
		const ImVec2 vecDistanceSize = F::VerdanaESP->CalcTextSizeA(12.f, FLT_MAX, 0.0f, szDistance.c_str());

		D::AddText(F::VerdanaESP, 12.f, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), szDistance, colPrimary, DRAW_TEXT_OUTLINE, colOutline);
...
void CVisuals::Player(CBaseEntity* pLocal, CBaseEntity* pEntity, Context_t& ctx, const float flDistance, const Color& colInfo, const Color& colFrame, const Color& colOutline)
{
	...
	if (C::Get<int>(Vars.iEspMainEnemyBox) > (int)EVisualsBoxType::NONE && pEntity->IsEnemy(pLocal))
	{
		// get box color based on visibility & enmity
		const Color colBox = pLocal->IsVisible(pEntity, pEntity->GetBonePosition(BONE_HEAD).value_or(pEntity->GetEyePosition(false))) ? C::Get<Color>(Vars.colEspMainBoxEnemies) : C::Get<Color>(Vars.colEspMainBoxEnemiesWall);

		Box(ctx.box, C::Get<int>(Vars.iEspMainEnemyBox), colBox, Color(0, 0, 0, 150));
	}

	if (C::Get<int>(Vars.iEspMainAllyBox) > (int)EVisualsBoxType::NONE && !pEntity->IsEnemy(pLocal))
	{
		// get box color based on visibility & enmity
		const Color colBox = pLocal->IsVisible(pEntity, pEntity->GetBonePosition(BONE_HEAD).value_or(pEntity->GetEyePosition(false))) ? C::Get<Color>(Vars.colEspMainBoxAllies) : C::Get<Color>(Vars.colEspMainBoxAlliesWall);

		Box(ctx.box, C::Get<int>(Vars.iEspMainAllyBox), colBox, Color(0, 0, 0, 150));
	}

	// info's master check
	if ((!C::Get<bool>(Vars.bEspMainAllyInfo) && !pEntity->IsEnemy(pLocal)) || (!C::Get<bool>(Vars.bEspMainEnemyInfo) && pEntity->IsEnemy(pLocal)))
		return;

	// @note: distance font scale
	const float flFontSize = std::clamp(60.f / (flDistance / 60.f), 10.f, 30.f);

#pragma region visuals_player_top
	if ((C::Get<bool>(Vars.bEspMainAllyFlash) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyFlash) && pEntity->IsEnemy(pLocal)) && pEntity->GetFlashDuration() > 0.2f)
		FlashBar(pEntity, ctx, Color(255, 255, 255, 220), Color(40, 40, 40, 100), Color(0, 0, 0, 150));

	if ((C::Get<bool>(Vars.bEspMainAllyRank) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyRank) && pEntity->IsEnemy(pLocal)) && !pInfo.bFakePlayer)
	{
		// @todo: make with images from vpk
	}

	if ((C::Get<bool>(Vars.bEspMainAllyName) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyName) && pEntity->IsEnemy(pLocal)))
	{
		// get player name
		...
		ImVec2 vecBotSize{ };
		const ImVec2 vecNameSize = F::VerdanaESP->CalcTextSizeA(flFontSize, FLT_MAX, 0.0f, szName.c_str());

		// add prefix for bots
		if (pInfo.bFakePlayer)
		{
			vecBotSize = F::VerdanaESP->CalcTextSizeA(flFontSize, FLT_MAX, 0.0f, szBot);

			D::AddText(F::VerdanaESP, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f + 1 + vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecBotSize.y - ctx.arrPadding.at(DIR_TOP)), szBot, C::Get<Color>(Vars.colColorHighlights), DRAW_TEXT_OUTLINE, colOutline);
		}

		D::AddText(F::VerdanaESP, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecNameSize.y - ctx.arrPadding.at(DIR_TOP)), szName, colInfo, DRAW_TEXT_OUTLINE, colOutline);
...
#pragma region visuals_player_bottom
	...
	{
		// ammo bar
		if ((C::Get<bool>(Vars.bEspMainAllyAmmo) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyAmmo)))
			AmmoBar(pEntity, pActiveWeapon, ctx, C::Get<Color>(Vars.colColorHighlights), Color(40, 40, 40, 100), Color(0, 0, 0, 150));

		// get all other weapons
		if ((C::Get<bool>(Vars.bEspMainAllyWeapons) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyWeapons)))
		{
	...
	if ((C::Get<bool>(Vars.bEspMainAllyDistance) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyDistance)))
	{
		const int iDistance = static_cast<int>(M_INCH2METRE(flDistance));
		std::string szDistance = std::to_string(iDistance).append(XorStr("M"));
		const ImVec2 vecDistanceSize = F::VerdanaESP->CalcTextSizeA(flFontSize, FLT_MAX, 0.0f, szDistance.c_str());
		D::AddText(F::VerdanaESP, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), szDistance, colInfo, DRAW_TEXT_OUTLINE, colOutline);
		ctx.arrPadding.at(DIR_BOTTOM) += vecDistanceSize.y;
	}
#pragma endregion

#pragma region visuals_player_left
	if ((C::Get<bool>(Vars.bEspMainAllyHealth) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyHealth)))
	{
	...
	if ((C::Get<bool>(Vars.bEspMainAllyMoney) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyMoney)))
	{
		std::string szMoney = std::to_string(pEntity->GetMoney()).insert(0U, XorStr("$"));
		const ImVec2 vecMoneySize = F::VerdanaESP->CalcTextSizeA(flFontSize, FLT_MAX, 0.0f, szMoney.c_str());
		D::AddText(F::VerdanaESP, flFontSize, ImVec2(ctx.box.left - 2 - vecMoneySize.x - ctx.arrPadding.at(DIR_LEFT), ctx.box.top), szMoney, Color(255, 255, 255), DRAW_TEXT_OUTLINE, colOutline);
		ctx.arrPadding.at(DIR_LEFT) += vecMoneySize.x;
	}
#pragma endregion

#pragma region visuals_player_right
	if ((C::Get<bool>(Vars.bEspMainAllyHelmet) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyHelmet)) && pEntity->HasHelmet())
	{
	...
	if ((C::Get<bool>(Vars.bEspMainAllyKevlar) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyKevlar)) && pEntity->GetArmor() > 0)
	{
	...
	if ((C::Get<bool>(Vars.bEspMainAllyDefuseKit) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyDefuseKit)) && pEntity->HasDefuser())
	{
	...
	if ((C::Get<bool>(Vars.bEspMainAllyZoom) && !pEntity->IsEnemy(pLocal)) || (C::Get<bool>(Vars.bEspMainEnemyZoom)) && pEntity->IsScoped())
	{
