...
bool H::Setup()
{
	...
	static void* pCSPlayerTable = MEM::GetVTablePointer(CLIENT_DLL, XorStr("C_CSPlayer"));

	if (!DTR::DoExtraBonesProcessing.Create(MEM::GetVFunc(pCSPlayerTable, VTABLE::DOEXTRABONESPROCESSING), &hkDoExtraBonesProcessing))
		return false;
...
void H::Restore()
{
	...
	DTR::DoExtraBonesProcessing.Remove();
...
bool FASTCALL H::hkCreateMove(IClientModeShared* thisptr, int edx, float flInputSampleTime, CUserCmd* pCmd)
{
	...
	CPrediction::Get().Start(pCmd, pLocal);
	{
		...
		{
			CAntiAim::Get().UpdateServerAnimations(pCmd, pLocal, bSendPacket);
			CAntiAim::Get().UpdateFakeAnimations(pLocal, bSendPacket);
			CAntiAim::Get().UpdateFakelagAnimations(pLocal, bSendPacket);
		}
	...
	// save menu highlight to gloabls
	G::vecMenuHighlight = C::Get<ImVec4>(Vars.vecColorHighlights);

	// save next view angles state
	G::angViewPoint = pCmd->angViewPoint;

	// save view point to global variables once everything is processed in createmove
	if (bSendPacket)
		G::angRealAngle = pCmd->angViewPoint;
	else
		G::angFakeAngle = pCmd->angViewPoint;
	...
	case FRAME_RENDER_START:
	{
		...
		*pLocal->GetFlashMaxAlpha() = C::Get<bool>(Vars.bWorld) ? C::Get<int>(Vars.iWorldMaxFlash) * 2.55f : 255.f;
		...
		if (C::Get<bool>(Vars.bWorld) && C::Get<int>(Vars.iWorldThirdPersonKey) > 0)
		{
			...
			if (bThirdPerson)
				I::Prediction->SetLocalViewAngles(G::angRealAngle);
		}
...
void FASTCALL H::hkDoExtraBonesProcessing(uintptr_t ecx, uintptr_t edx, CStudioHdr* hdr, Vector* pos, Quaternion* q, matrix3x4_t* matrix, c_bone_bit_list& bone_list, c_ik_context* context)
{
	auto e = reinterpret_cast<CBaseEntity*>(ecx);

	static auto oOriginal = DTR::DoExtraBonesProcessing.GetOriginal<decltype(&hkDoExtraBonesProcessing)>();

	const auto state = e->GetAnimationState();

	if (!state || !state->pEntity)
		return oOriginal(ecx, edx, hdr, pos, q, matrix, bone_list, context);
	...
	oOriginal(ecx, edx, hdr, pos, q, matrix, bone_list, context);
	std::swap(*reinterpret_cast<int*>(uintptr_t(state) + 0x8), backup_tickcount);
}
